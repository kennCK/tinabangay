  ...
  <div v-if="data !== null" class="holder w-100">
    <input type="text" class="form-control" v-model="searchValue" placeholder="Search location" @keyup="filterLocation()">
      <div class="row justify-content-end m-0 mr-5 align-items-center">
        <button class="btn btn-primary ml-2 mr-auto mb-3" @click="showModal()">Add Hot Spot</button>
...
export default {
  ...
  methods: {
    ...
    showModal() {
      ...
      let input = this.modalProperty.inputs = this.modalProperty.inputs.filter(input => input.variable === 'location')
      $('#createHotSpotModal').modal('show')
    },
    filterLocation(){
