    ...
    showModal(modal, account = null, patientID = null, update = false){
      if(modal === 'patient') {
        ...
        $('<button>', {
          ...
          style: 'margin: .5% 0',
          html: 'No Account'
        }).preppend('#createPatientsModal .modal-body')
        inputs.map(input => {
        ...
        $('.modal-footer').css({display: 'none'})
        $('#createPatientsModal label[for=username]').html('Username <small>(hit enter when you\'re done)')
        $('#username').keypress((e) => {
          if(e.keyCode === 13) {
            let userParams = {
              condition: [{
                  ...
                  let item = $('<div>', {
                    ...
                    click: () => {
                      ...
                      inputs.map(par => {
                        ...
                        } else if (par.variable === 'status') {
                          par.type = 'select_specified'
                          par.inputType = null
                        } else {
                          par.inputType = 'text'
                        }
